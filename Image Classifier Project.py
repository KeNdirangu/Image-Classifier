...
train_transforms = transforms.Compose([transforms.RandomResizedCrop(size=224), #load and tranfrom training images
                                     transforms.RandomRotation(30),
                                     transforms.ToTensor(),
                                     transforms.RandomHorizontalFlip(),
                                     transforms.Normalize([0.485,0.456,0.406],[0.229,0.224,0.225])])

test_transforms = transforms.Compose([transforms.Resize(256), #Load and transfrom test images
                                     transforms.CenterCrop(224),
                                     transforms.ToTensor(),
                                     transforms.Normalize([0.485,0.456,0.406],[0.229,0.224,0.225])])

validation_transforms = transforms.Compose([transforms.Resize(256), #load and transfrom validation data
                                     transforms.CenterCrop(224),
                                     transforms.ToTensor(),
                                     transforms.Normalize([0.485,0.456,0.406],[0.229,0.224,0.225])])

# TODO: Load the datasets with ImageFolder
Tr_data = datasets.ImageFolder(data_dir + '/train', transform=train_transforms) #create a data folder for transformed images
...
trloader = torch.utils.data.DataLoader(Tr_data, batch_size = 64, shuffle = True) #load transformed data with shuffling of images to enhance training
tsLoader = torch.utils.data.DataLoader(Ts_data, batch_size = 32, shuffle = True)
vdLoader = torch.utils.data.DataLoader(Vd_data, batch_size = 24, shuffle = True)

#adopted from colab.research.google.com/drive/1hf8WzbpApgHNgNj9GrwT9U14IIVPI_Dg?usp=sharing
...
#cadopted from colab.research.google.com/drive/1hf8WzbpApgHNgNj9GrwT9U14IIVPI_Dg?usp=sharing
#selected vgg16 - robust weight parameters and relatively versatile compared to other models, 16 layers also ideal for learning
model = models.vgg16(weights = True)
model
#freeze feature parameters - speed up training by preventing gradient updating in each pass
for param in model.parameters():
    param.requires_grad = False

#defining the classifier
from collections import OrderedDict #to set order in which keys are inserted in our classifier

classifier = nn.Sequential(OrderedDict([('inputs', nn.Linear(25088, 4096)), #define model including hidden layers & hyperparameters
                                       ('relu1', nn.ReLU()),
...
correct, total = 0,0
# adopted from https://katba-caroline.com/wp-content/uploads/2018/11/Image-Classifier-Project.html
with torch.no_grad():
    model.eval()
...
#practice image
prac_image = (data_dir + '/test' + '/19/' + 'image_06186.jpg')
...
imshow(process_image(prac_image))
...
#adopted from https://github.com/CaterinaBi/udacity-image-classifier/blob/main/image_classifier_project.ipynb
# Done: Display an image along with the top 5 classes
imshow(process_image(practice_img))
plt.figure()
plt.barh(cats,width=probs)
